...
/**
 * Converts a collection of (oriented) links to a collection of their heads
 */
public class LinksToHeadsConverter extends CollectionToCollectionConverter<Link, Vertex> {

    private final OrientedGraphAnalyzer graphAnalyzer;

    public LinksToHeadsConverter(OrientedGraphAnalyzer graphAnalyzer) {
        this.graphAnalyzer = graphAnalyzer;
    }

    @Override
    protected Vertex convertItem(Link link) {
        return graphAnalyzer.getHead(link);
    }
